...
`saad-node-agent` is a small, telemetry-only Python service installed on each Linux CRM node. It reports host health, discovered `saad-deploy` applications, deployment state, and Docker resources to `saad-dashboard` every 15 seconds by default.
...
The Python service runs as the unprivileged `saad-node-agent` user and does not have direct access to `/var/run/docker.sock`. systemd reads the root-owned `0600` config file and supplies its values as process environment; the agent does not need to reopen the secret file. It can run exactly one `sudo -n` command: the root-owned, group-executable `/usr/local/lib/saad-node-agent/snapshot.sh` with no arguments. The associated sudoers entry is argument-free.

That helper has a fixed `PATH`, accepts no user input, and only runs these Docker read operations: `docker ps --all --quiet`, `docker inspect`, and `docker stats --all --no-stream`. It returns their data as JSON. There is no generic shell-execution feature or dashboard-to-server command path.

The systemd unit additionally limits filesystem visibility, capabilities, address families, and file permissions. Deployment config and state are read only; no local database is created.
